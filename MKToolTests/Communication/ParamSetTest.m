...
@interface ParamSetTest : GHTestCase
...
- (void)testInvalidRevision {
  ...
  GHAssertNotNil(origPayload, @"Payload creating failed");

  GHAssertThrows([IKParamSet settingWithData:origPayload], @"");
}
...
- (void)testParamset92 {
  ...
  GHAssertNotNil(p, @"IKParamSet creating failed");

  NSString *name = [NSString stringWithCString:pMk.Name encoding:NSASCIIStringEncoding];
  GHAssertNotNil(p.Name, @"IKParamSet name");

  GHAssertEqualObjects(name, p.Name, @"The names are not equal");

  GHAssertEqualObjects(origPayload, [p data], @"The encoded data is not equal");
}
